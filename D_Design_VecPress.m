clc
clear
load Velocity_opt.mat Ubaser Atilde B1tilde B2tilde B3tilde Y_Fish PARA Nx Ny
lx = Nx; ly = Ny;
X = Y_Fish;
r = 200;
...
VSec_crit = SVmax/max(SVmax);

[SV_sort,SV_index] = sort(VSec_crit,'descend');


load Pressure_opt.mat Ubaser Atilde B1tilde B2tilde B3tilde Y_Fish PARA Nx Ny
lx = Nx; ly = Ny;
X = Y_Fish;
r = 200;
%% Evaluate initial conditions
...
Set = [1 4 5 8 9 10 12 14 15]; %[1 2 4 8 10 12 14 15];
[U,Sigma,V] = svd(S(Set,:),'econ');      % Step 1
% assignin('base', 'M', M);
rp = 4;
Ur = U(:,1:rp);
...
PSec_crit = SPmax/max(SPmax);

[SP_sort,SP_index] = sort(PSec_crit,'descend');
...
P_pareto = P(isPareto,:); % parteo parameters

[Fsort, Findex] = sort(F_pareto(:,1));

plot(Vec_crit(:), Pre_crit(:), 'bo'); hold on
